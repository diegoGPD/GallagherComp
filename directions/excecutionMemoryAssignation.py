...
def initVirtualMemory():
    ...
    for mainVarial in virtualMemory['local']['main']:
        if (mainVarial):
            if isinstance(virtualMemory['local']['main'][mainVarial], list):
                mainObject[virtualMemory['local']['main'][mainVarial][0]] = None
            else:
...
def addTocVirtualMemory(tocName):
    tocObjecto = {}
    for tocVarial in virtualMemory['local'][tocName]:
        tocObjecto[virtualMemory['local'][tocName][tocVarial]] = None
    excecutionMemory['local'].append(tocObjecto)
    advanceExcectuionPointer()
